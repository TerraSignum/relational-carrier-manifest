...
reviewer wanting the architecture before the technical layers.
\end{abstract}
...
validation is the principal next step. \emph{No claim made here
is novel}: every numerical value, every closure form, every
falsification handle is sourced to the technical paper that owns
it. If a sentence in this guide and a sentence in the cited paper
disagree, the cited paper is authoritative.
...
\esync\!=\!1/(4(d{+}1)),\;
\bpi\!=\!15/16,\;
\DOmega\!=\!1\!-\!(N_{\rm gen}d{+}1)/(d^{2}(d{+}1))$. Both $d$
and $N_{\rm gen}$ are observationally fixed integer counts of
discrete-geometric features of the underlying relational
lattice, not free parameters. The free-parameter count of the
structural input is therefore zero. The five System-$\Rsys$
...
closure ($v_{\rm EW}\!=\!246.22$~GeV via P1~\cite{Paper1}'s
...
(EXACT $\le 1\%$, PRECISE $\le 2.5\%$, FACTOR2 within $2\times$).
...
$\Sigma m_{\nu}$ & $\gamma^{2}m_{3}\!+\!m_{2}\!+\!m_{3}\!\simeq\!0.060$ eV & PRECISE & downstream & P6 \\
\midrule
\multicolumn{5}{c}{\textit{Electroweak \& Standard Model}} \\
$v_{\rm EW}$ & 4-path consensus $246.22$ GeV & PRECISE & load-bearing input & P1 \\
$\alpha_{\rm EM}$ & $\gamma^{2}\axi^{N_{\rm gen}}\!=\!729/10^{5}$ & EXACT & downstream & P3 \\
...
$m_{\mu}/m_{e}$ & spectral Yukawa chirality split $\to 206.0$ & EXACT & downstream & P3 \\
...
$\bar\theta_{\rm QCD}\!=\!0$ & APS-chirality pairing & STRUCTURAL & downstream & P4A \\
\bottomrule
\end{longtable}
...
within the BICEP/Keck BK18 upper bound
...
(\url{https://github.com/[anonymized]/relational-carrier-manifest}) bundles all
