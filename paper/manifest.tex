...
\pyscript{relational-uv-closure-repro}{data/corpus_claims_manifest.json}{corpus_claims_manifest.json} in P6's
...
normalised graph Laplacian, with branched asymptotes
$\lambda_{\infty}^{(\rm vac)}\!=\!3/8\!=\!(d{-}1)/(2d)$ (vacuum,
Bayesian posterior $0.908$ on the $N \leq 100$ ladder) and
$\lambda_{\infty}^{(\rm mat)}\!=\!79/200\!=\!3/8\!+\!d\gamma^{2}/2$
(matter-stable, $256\leq N \leq 600$, $-0.16\%$ relative residual,
Bayes-factor $S_{2}/S_{1}\!=\!0.999$). Both closures hold
...
extension of the empirical certification to higher $N$ requires
the standard lattice-continuum parameter rescaling that preserves
$\theta_{\rm chir}$ on its canonical trajectory. The fixed-parameter
$N > N_{\rm inv}$ regime exits the canonical sequence (chirality
saturates at $\arctan(N_{\rm gen})$, lattice exhibits uniform
$\xi$-densification with a drifting non-asymptotic spectral gap;
P4~\cite{Paper4}~Remark~\texttt{rmk:longer\_iter\_accelerating}) and
is a numerical artefact of fixed-parameter ladder construction,
not a falsification of the $(\mathrm{SG})$ axiom. The
astrophysical and cosmological observable scales of the corpus
realise the canonical sequence and are unaffected. Neither
branch is analytically discharged at theorem level. A GPU
...
\pyscript{causal-wave-landings-repro}{RELEASE_v1_0_0.md}{RELEASE_v1_0_0.md} with full release notes in
\pyscript{causal-wave-landings-repro}{CHANGELOG.md}{CHANGELOG.md}. Software metadata is in \path|pyproject.toml|
